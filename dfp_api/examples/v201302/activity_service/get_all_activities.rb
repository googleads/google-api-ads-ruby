...
def get_all_activities()
  ...
  activity_group_ids = get_all_activity_group_ids(activity_group_service)
  total_results_counter = 0

  activity_group_ids.each do |activity_group_id|
    # Define initial values.
    offset = 0
    page = {}
    query_base = 'WHERE activityGroupId = :activityGroupId AND ' +
    ...
    if page.include?(:total_result_set_size)
      total_results_counter += page[:total_result_set_size]
    end
  end

  # Print a footer.
  puts "Total number of results: %d" % total_results_counter
end

def get_all_activity_group_ids(activity_group_service)
  # Define initial values.
  offset = 0
  page = {}
  activity_group_ids = []
  ...
  begin
    ...
    if page[:results]
      ...
      activity_group_ids |=
          page[:results].map {|activity_group| activity_group[:id]}
    end
  end while offset < page[:total_result_set_size]
