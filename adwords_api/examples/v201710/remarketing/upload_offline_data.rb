...
def upload_offline_data(conversion_name, external_upload_id,
  ...
  return_value = offline_data_upload_srv.mutate([data_upload_operation])
  ...
  # request.
  unless offline_data_upload[:partial_data_errors].nil?
    offline_data_upload[:partial_data_errors].each_with_index do |api_error, i|
      next if api_error.nil?
      puts ("Found a partial error for offline data #%d with error " +
          "string: '%s'") % [i, api_error[:error_string]]
    end
  end
end
