...
require 'targeting/add_demographic_targeting_criteria'
...
class TestTargetingV201302 < Test::Unit::TestCase
  def setup
  ...
  def test_add_demographic_targeting_criteria
    ad_group = @utils.get_ad_group()
    assert_not_nil(ad_group)
    assert_not_nil(ad_group[:id])
    add_demographic_targeting_criteria(ad_group[:id])
  end
