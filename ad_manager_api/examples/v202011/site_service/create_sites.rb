...
def create_sites(ad_manager, url, child_network_code)
  # Get the NetworkService and SiteService.
  network_service = ad_manager.service(:NetworkService, API_VERSION)
  site_service = ad_manager.service(:SiteService, API_VERSION)

  # Check that the child network is approved for MANAGE_INVENTORY by this
  # network.
  current_network = network_service.get_current_network()
  child_network = current_network[:child_publishers].find do |child|
    child[:child_network_code] == child_network_code
  end

  if child_network.nil?
    raise ('Child network %s not found on the current network (%s). Cannot ' +
        'create site.') % [child_network_code, current_network[:network_code]]
  end

  if child_network[:approved_delegation_type] != 'MANAGE_INVENTORY'
    raise ('Child network %s has not approved for the current network (%s) ' +
        'to manage its inventory. Cannot create site.') % [child_network_code,
        current_network[:network_code]]
  end
