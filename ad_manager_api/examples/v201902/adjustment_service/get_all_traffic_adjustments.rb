...
def get_all_traffic_adjustments(ad_manager)
  ...
  begin
    page = adjustment_service.get_traffic_adjustments_by_statement(
    ...
    unless page[:results].nil?
      page[:results].each_with_index do |adjustment, index|
        puts ('Traffic forecast adjustment with id %d and %d segments was ' +
            'found.') % [adjustment[:id],
            adjustment[:forecast_adjustment_segments].to_a.size]
      end
    end
  ...
  puts 'Total number of traffic forecast adjustments: %d' %
...
if __FILE__ == $0
  ...
  begin
    get_all_traffic_adjustments(ad_manager)
