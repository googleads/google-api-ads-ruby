...
def get_all_forecast_adjustments(ad_manager)
  ...
  begin
    ...
    unless page[:results].nil?
      page[:results].each_with_index do |adjustment, index|
        puts ('Forecast adjustment with id %d and name "%s" belonging to ' +
            'segment %d was found.') % [adjustment[:id], adjustment[:name],
            adjustment[:traffic_forecast_segment_id]]
      end
    end
...
if __FILE__ == $0
  ...
  begin
    get_all_forecast_adjustments(ad_manager)
